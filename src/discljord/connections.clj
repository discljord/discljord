...
;; NOTE(Joshua): right now all this connection code is synchronous,
;;               and that's hurting the project quite a bit. Stuff is trying
;;               to connect many times with the same shard on different
;;               go blocks. Need to change how connections work so that
;;               calling connect or reconnect will just put connecting
...
(defn on-connect
  [gateway token shard-id socket-state]
  (println "Connected!")
...
(defn on-receive
  [msg gateway token shard-id event-channel socket-state]
...
(defn on-close
  [stop-code msg gateway token shard-id event-channel socket-state]
